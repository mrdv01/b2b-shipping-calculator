    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
        <div className="flex items-center gap-2 mb-5">
          <h2 className="text-lg font-semibold text-white">Full Calculation</h2>
        </div>

        <div className="mb-4 px-4 py-3 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-sm text-cyan-300 font-mono">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 px-4 bg-gradient-to-r from-accent to-primary text-white font-medium rounded-xl hover:shadow-lg hover:shadow-accent/25 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            {loading ? (
      ...
      <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
        ...
        {result && (
          <div className="space-y-4">
            {/* Main charge */}
            <div className="p-5 rounded-xl bg-gradient-to-br from-accent/20 to-primary/10 border border-accent/20 text-center">
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-slate-300 px-1">Nearest Warehouse</p>
              <ResultRow label="ID" value={result.nearestWarehouse.warehouseId} />
              <ResultRow label="Name" value={result.nearestWarehouse.warehouseName} />
              <ResultRow
                label="Location"
                value={`${result.nearestWarehouse.warehouseLocation.lat}, ${result.nearestWarehouse.warehouseLocation.long}`}
              />
            </div>
